<template><div><h1 id="服务器" tabindex="-1"><a class="header-anchor" href="#服务器" aria-hidden="true">#</a> 服务器</h1>
<p><strong>## 1.问题描述</strong></p>
<p>需求：监听文件变化自动将文件上传到服务器，但是需要配置服务器ssh 公匙列表</p>
<p>服务器查看匹配的公匙列表<code v-pre>cd ~/.ssh</code>  然后打开authorized_keys 这里面记录了对应的公钥，本地主机的公钥一般放在C/User/.ssh/id_rsa.pub。将本地的pub文件内容粘贴到authorized_keys 里面完成配置。</p>
<p>~~~shell</p>
<p>服务器查看匹配的公匙列表<code v-pre>cd ~/.ssh</code></p>
<p>cd ~/.ssh</p>
<p>文件authorized_keys 记录公钥列表</p>
<p>这个代码是将本地的dist上传到服务器的/data/my_docs文件夹中</p>
<p>scp -r E:\360MoveData\Users\YR\Desktop\Blog\docs.vuepress\dist root@43.138.53.141:/data/my_docs</p>
...
<p>~~~</p>
<h2 id="服务器搭建流程" tabindex="-1"><a class="header-anchor" href="#服务器搭建流程" aria-hidden="true">#</a> 服务器搭建流程</h2>
<p>1.使用SSH工具连接到你的云服务器，运行挂载命令：</p>
<p>2.执行宝塔安装命令</p>
<p>Centos安装命令：</p>
<blockquote>
...
<blockquote>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">wget</span> <span class="token parameter variable">-O</span> install.sh https://download.bt.cn/install/install-ubuntu_6.0.sh <span class="token operator">&amp;&amp;</span> <span class="token function">sudo</span> <span class="token function">bash</span> install.sh
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></blockquote>
<p>Debian安装命令：</p>
<blockquote>
