...
<p>Vite 以 <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules" target="_blank" rel="noopener noreferrer">原生 ESM<ExternalLinkIcon/></a> 方式提供源码。这实际上是让浏览器接管了打包程序的部分工作：Vite 只需要在浏览器请求源码时进行转换并按需提供源码。根据情景动态导入代码，即只在当前屏幕上实际使用时才会被处理。借用官方的图,我们可以看到Vite是按需加载，传统的打包工具如<code v-pre>Webpack</code>是先解析依赖、打包构建再启动开发服务器，<code v-pre>Dev Server</code> 必须等待所有模块构建完成，当我们修改了 <code v-pre>bundle</code>模块中的一个子模块， 整个 <code v-pre>bundle</code> 文件都会重新打包然后输出。项目应用越大，启动时间越长。而<code v-pre>Vite</code>利用浏览器对<code v-pre>ESM</code>的支持，当 <code v-pre>import</code> 模块时，浏览器就会下载被导入的模块。先启动开发服务器，当代码执行到模块加载时再请求对应模块的文件,本质上实现了动态加载。灰色部分是暂时没有用到的路由，所有这部分不会参与构建过程。随着项目里的应用越来越多，增加<code v-pre>route</code>，也不会影响其构建速度。</p>
<p><img src="/Vite/vite.png" alt=""></p>
<h3 id="vite快在哪" tabindex="-1"><a class="header-anchor" href="#vite快在哪" aria-hidden="true">#</a> Vite快在哪</h3>
<p>Vite利用服务器原生支持ESM在服务端按需编译提供代码。</p>
<p>1.快速的冷启动: <code v-pre>No Bundle</code> + <code v-pre>esbuild</code> 预构建   dev server`</p>
<p>2.即时的模块热更新: 基于<code v-pre>ESM</code>的<code v-pre>HMR</code>，同时利用浏览器缓存策略提升速度 <code v-pre>HMR</code></p>
<p>3.真正的按需加载: 利用浏览器<code v-pre>ESM</code>支持，实现真正的按需加载</p>
<p>接下来解释几个概念</p>
<div class="custom-container tip"><p class="custom-container-title">TIP</p>
<p><code v-pre>ESM</code>的执行可以分为三个步骤：</p>
<ul>
<li>
<p>构建: 确定从哪里下载该模块文件、下载并将所有的文件解析为模块记录</p>
</li>
<li>
<p>实例化: 将模块记录转换为一个模块实例，为所有的模块分配内存空间，依照导出、导入语句把模块指向对应的内存地址。</p>
</li>
<li>
<p>运行：运行代码，将内存空间填充</p>
</li>
...
<div class="custom-container tip"><p class="custom-container-title">TIP</p>
<p><code v-pre>Esbuild</code>是一个<code v-pre>JavaScript``  Bundler </code>打包和压缩工具，它提供了与<code v-pre>Webpack</code>、<code v-pre>Rollup</code>等工具相似的资源打包能力。可以将<code v-pre>JavaScript</code> 和<code v-pre>TypeScript</code>代码打包分发在网页上运行。但其打包速度却是其他工具的10～100倍。它提供了一些API：<code v-pre>Transform、build、context</code>https://esbuild.github.io/api/#overview详情看官方文档</p>
</div>
<p><code v-pre>Vite</code>其核心原理是利用浏览器现在已经支持<code v-pre>ES6</code>的<code v-pre>import</code>,碰见<code v-pre>import</code>就会发送一个<code v-pre>HTTP</code>请求去加载文件，<code v-pre>Vite</code>启动一个 <code v-pre>koa</code> 服务器拦截这些请求，并在后端进行相应的处理将项目中使用的文件通过简单的分解与整合，然后再以<code v-pre>ESM</code>格式返回返回给浏览器。<code v-pre>Vite</code>整个过程中没有对文件进行打包编译，做到了真正的按需加载，所以其运行速度比原始的<code v-pre>webpack</code>开发编译速度快出许多！同时，<code v-pre>Vite</code> 还利用<code v-pre>HTTP</code>加速整个页面的重新加载。设置响应头使得依赖模块(<code v-pre>dependency module</code>)进行强缓存，而源码文件通过设置 <code v-pre>304 Not Modified</code> 而变成可依据条件而进行更新。</p>
<h2 id="_2-hmr热更新" tabindex="-1"><a class="header-anchor" href="#_2-hmr热更新" aria-hidden="true">#</a> 2.HMR热更新</h2>
<p>为什么需要HMR?HMR是什么？对于开发体验来说，我们在开发时修改了代码中的某处，如果我们要想看到效果怎么办？是不是要刷新以下浏览器？这样有什么后果？我们之前的状态全部被初始化。而热更新能够做到在不刷新浏览器的情况下，保持浏览器使用的是我们最新的代码，让开发者可以立即看到所做的更改在浏览器中的反应。</p>
<p>目前所有的打包工具实现热更新的思路都大同小异：主要是通过<code v-pre>WebSocket</code>创建浏览器和服务器的通信监听文件的改变，当文件被修改时，服务端发送消息通知客户端修改相应的代码，客户端对应不同的文件进行不同的操作的更新。</p>
<p><code v-pre>Vite</code>整个热更新过程可以分成五步</p>
<ol>
<li>修改代码，vite server 监听到代码被修改</li>
<li>vite 计算出热更新的边界（即受到影响，需要进行更新的模块）</li>
<li>vite server 通过 websocket 告诉 vite client 需要进行热更新</li>
<li>浏览器拉取修改后的模块</li>
<li>执行热更新的代码</li>
</ol>
<h3 id="_2-1、vite-client客户端" tabindex="-1"><a class="header-anchor" href="#_2-1、vite-client客户端" aria-hidden="true">#</a> 2.1、vite-client客户端</h3>
<p>主要涉及两个api  <code v-pre>hot.dispose</code> 和 <code v-pre>hot.accept</code></p>
<p>当我们修改内容的时候accept回调就会执行，并重新执行render函数来更新页面</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">if</span> <span class="token punctuation">(</span><span class="token keyword">import</span><span class="token punctuation">.</span>meta<span class="token punctuation">.</span>hot<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>dispose</code>主要是做善后工作，老模块的定时器清理。</p>
<p>vite开发服务器会在index.html注入一个@vite/client的脚本，当访问index.html时就会拉取该脚本，该脚本会创建<code v-pre>websocket</code></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">//监听服务端的消息</span>
...
<span class="token keyword">async</span> <span class="token keyword">function</span> <span class="token function">handleMessage</span><span class="token punctuation">(</span><span class="token parameter">payload</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2-2、热更新边界" tabindex="-1"><a class="header-anchor" href="#_2-2、热更新边界" aria-hidden="true">#</a> 2.2、热更新边界</h3>
<p>即一个文件被修改，vite会沿着依赖树向上找最近的一个可以热更新模块作为热更新边界，然后对其进行热更新。而对于没有上层可热更新的模块就会刷新页面，例如在vue项目中main.ts文件。当我改了vue中的style代码后浏览器拉取的结果</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> createHotContext <span class="token keyword">as</span> __vite__createHotContext <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">"/@vite/client"</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_3-预构建" tabindex="-1"><a class="header-anchor" href="#_3-预构建" aria-hidden="true">#</a> 3.预构建</h2>
<p>Vite使用esbuild进行预构建。Vite将预编译后的文件缓存到<code v-pre>node_modules/.vite</code>文件夹下（一般是第三方依赖），由于预编译的文件已经被打包和优化过了，这样浏览器再加载这些文件的时候不需要再构建直接将预构建的文件给浏览器。</p>
<h3 id="_3-1预构建的意义" tabindex="-1"><a class="header-anchor" href="#_3-1预构建的意义" aria-hidden="true">#</a> 3.1预构建的意义</h3>
<ol>
<li>为了兼容<code v-pre>commenJS</code>，我们知道Vite使用<code v-pre>ESM</code>，那么Vite必须将使用commenJS或UMD规范的依赖转成ESM</li>
<li>模块之间有众多依赖关系，Vite将依赖关系转成单个模块来提高后续加载性能，说白了为了减少模块和请求数量。什么意思？例如，<code v-pre>lodsh-es</code>,当我们执行 <code v-pre>import { debounce } from 'lodash-es'</code> 时，浏览器同时发出 600 多个 HTTP 请求！尽管服务器在处理这些请求时没有问题，但大量的请求会在浏览器端造成网络拥塞，导致页面的加载速度相当慢。通过预构建 <code v-pre>lodash-es</code> 成为一个模块，我们就只需要一个 HTTP 请求了。</li>
</ol>
<h3 id="_3-2如何进行预构建" tabindex="-1"><a class="header-anchor" href="#_3-2如何进行预构建" aria-hidden="true">#</a> 3.2如何进行预构建</h3>
<p><strong>①依赖扫描</strong>：找出第三方依赖，并进行键值对形式存储。然后将这些第三方依赖先进行构建。</p>
<p>vite从入口的 HTML 文件开始，根据模块的 import 依赖关系，可以连接成一颗模块依赖树。要扫描出所有的 bare import，就需要遍历整个依赖树，这就涉及到了<strong>树的深度遍历</strong>，我们只需要深度遍历所有树节点，<strong>找出所有 import 语句，把 import 的模块记录下来</strong>即可，对于第三方依赖会被记录，对于开发者的模块会继续进行遍历。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>②打包依赖</strong>：依赖扫描已经拿到了所有需要预构建的依赖信息，那接下来直接使用 esbuild 进行打包即可。</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> build <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'esbuild'</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="/Vite/vite2.png" alt=""></p>
<h3 id="_3-3路径替换" tabindex="-1"><a class="header-anchor" href="#_3-3路径替换" aria-hidden="true">#</a> 3.3路径替换</h3>
<p>我们的裸模块使用的是Nodejs的模块查找方法（去node_modules中查找），而浏览器不理解，因此需要把<code v-pre>import vue</code>转成<code v-pre>import “/node_modules/.vite/deps/vue.js”</code>,这样浏览器在处理到import时会给vite服务器发送请求，请求该文件，然后vite服务器中间件处理后将模块给浏览器。</p>
<h2 id="_4-vite如何处理页面资源" tabindex="-1"><a class="header-anchor" href="#_4-vite如何处理页面资源" aria-hidden="true">#</a> 4.vite如何处理页面资源</h2>
<p>所有的类 JS 模块（包括 Vue），CSS 模块，都会在 <code v-pre>transfrom</code> 中间件中进行处理</p>
<p>每个模块都会经过 <code v-pre>resolveId</code>、<code v-pre>load</code>、<code v-pre>transform</code> 三个流程，这三个流程，可以通过 <a href="https://link.juejin.cn?target=https%3A%2F%2Fcn.vitejs.dev%2Fguide%2Fapi-plugin.html%23universal-hooks" target="_blank" rel="noopener noreferrer">Vite 插件<ExternalLinkIcon/></a>去扩展，可以在这三个过程中做一些特殊处理</p>
<ul>
<li><code v-pre>html</code> 处理中间件，通过调用插件的 <code v-pre>transformIndexHtml</code> 对 <code v-pre>html</code> 页面进行处理。</li>
<li><code v-pre>TS</code> 处理中间件，通过调用插件的 <code v-pre>resolveId</code>、<code v-pre>load</code>、<code v-pre>transform</code> 这三个钩子，对代码进行处理的</li>
</ul>
<h2 id="_4-缓存机制" tabindex="-1"><a class="header-anchor" href="#_4-缓存机制" aria-hidden="true">#</a> 4.缓存机制</h2>
<p>Vite并使用 HTTP 304 Not Modified 状态码来指示浏览器使用缓存的资源。这样可以避免不必要的资源下载和带宽消耗。</p>
<p>当浏览器请求一个静态资源时，Vite会在服务器端检查该资源是否已被缓存。如果该资源已被缓存，Vite会在响应头中设置 Last-Modified 和 <code v-pre>ETag</code> 两个字段，并使用 <code v-pre>HTTP 304 Not Modified</code> 状态码返回响应。这告诉浏览器可以使用缓存的资源，而无需重新下载该资源。.</p>
<p>在后续的请求中，浏览器会将 Last-Modified 和 ETag 字段的值与服务器端缓存的值进行比较，以判断资源是否已被修改。如果资源未被修改，Vite会返回 HTTP 304 Not Modified 状态码，告诉浏览器可以使用缓存的资源。如果资源已被修改，Vite会返回新的资源内容和响应码200 OK。</p>
<p><img src="/Vite/vite3.png" alt=""></p>
<p>当修改文件后</p>
<p><img src="/Vite/vite4.png" alt=""></p>
<h2 id="_5-总结" tabindex="-1"><a class="header-anchor" href="#_5-总结" aria-hidden="true">#</a> 5.总结</h2>
<ol>
<li>Vite开发环境利用esbuild进行预构建。（这也是Vite首屏加载慢的原因）</li>
<li>Vite利用浏览器原生esm，实现按需加载，在加载资源时用中间件拦截然后处理</li>
<li>Vite将静态资源进行缓存，并使用热更新。</li>
</ol>
<h2 id="_6-vite性能优化" tabindex="-1"><a class="header-anchor" href="#_6-vite性能优化" aria-hidden="true">#</a> 6.Vite性能优化</h2>
<p>构建分析</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// npm install rollup-plugin-visualizer -D</span>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token function">defineConfig</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  ...
  <span class="token punctuation">]</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>①分包 这里包括路由懒加载</p>
<p>②Treeshaking</p>
<p>③gzip压缩</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// npm i vite-plugin-compression -D</span>
<span class="token comment">// vite.config.js</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> defineConfig <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'vite'</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>④CDN</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// npm i vite-plugin-cdn-import -D</span>
<span class="token comment">// vite.config.js</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> defineConfig <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'vite'</span>
...
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// npm i vite-plugin-imagemin -D</span>
<span class="token comment">// vite.config.js</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> defineConfig <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'vite'</span>
