...
<td>出队方式</td>
<td>先进先出</td>
<td>允许插队，按任务id从小大排列执行</td>
<td>按任务id从小大排列执行</td>
</tr>
</tbody>
</table>
<h2 id="pre队列任务" tabindex="-1"><a class="header-anchor" href="#pre队列任务" aria-hidden="true">#</a> pre队列任务</h2>
<p>这个队列的任务只有<code v-pre>watch</code>和<code v-pre>watchEffect</code>，当它两没有指定<code v-pre>{post}</code>时默认就是pre</p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">function</span> <span class="token function">doWatch</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
