<template><div><h1 id="vue-loader初探" tabindex="-1"><a class="header-anchor" href="#vue-loader初探" aria-hidden="true">#</a> vue-loader初探</h1>
<h2 id="_1-vue-loader" tabindex="-1"><a class="header-anchor" href="#_1-vue-loader" aria-hidden="true">#</a> 1.vue-loader</h2>
<p>vue-loader使用vue/complier-sfc包中的compiler来解析模板、脚本和样式，</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token keyword">function</span> <span class="token function">loader</span><span class="token punctuation">(</span>
...
<span class="token punctuation">)</span> <span class="token punctuation">{</span>
   ...
   <span class="token operator">...</span><span class="token operator">...</span><span class="token punctuation">.</span> 
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>vueloader做了什么：</p>
<ol>
<li>
<p>如果VueLoaderPlugin插件没有安装的情况下报错，==这个插件动态的添加loader处理从vue文件分割出来的js html css代码==</p>
<ol>
<li><img src="/Vue/vueloader1.png" alt=""></li>
<li>pitcher处理js 其他的处理html  css，最终重新定义loader的rules</li>
</ol>
</li>
<li>
<p>得出请求的路径 例如 import APP from './APP.vue'  ./APP.vue</p>
</li>
<li>
<p>取出参数</p>
</li>
<li>
<p>环境判断 生产环境？ node环境？</p>
</li>
<li>
<p>解析内容将 vue文件 模板 脚本 样式拆分成对象放到descriptor中</p>
</li>
<li>
<p>MAP结构缓存描述符</p>
<ol>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token operator">*</span> <span class="token keyword">as</span> fs <span class="token keyword">from</span> <span class="token string">'fs'</span>
...
<li>
<p>getDescriptor这个函数只会在描述符之后调用，如果没有命中cache缓存可能是由于加载器在单独的线程中运行。处理这件事唯一办法直接从磁盘读取该文件</p>
</li>
</ol>
</li>
<li>
<p>拿文件路径并生成hash作为id-》热重载  当文件内容变化重新生成新的hash</p>
</li>
<li>
<p>查看是否包含scoped已经热重载功能是否需要 。（生产环境node或者选项hotReload=false都不需要以及这三个条件其中一个为真descriptor.script || descriptor.scriptSetup || descriptor.template）</p>
</li>
<li>
<p>const propsToAttach: [string, string][] = [] 定义变量，给脚本添加额外属性来为Tree-shaking做准备</p>
</li>
<li>
<p>分割vue文件 script  template  style，提取参数。</p>
<ol>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> query <span class="token operator">=</span> <span class="token template-string"><span class="token template-punctuation string">`</span><span class="token string">?vue&amp;type=script</span><span class="token interpolation"><span class="token interpolation-punctuation punctuation">${</span>attrsQuery<span class="token interpolation-punctuation punctuation">}</span></span><span class="token interpolation"><span class="token interpolation-punctuation punctuation">${</span>resourceQuery<span class="token interpolation-punctuation punctuation">}</span></span><span class="token template-punctuation string">`</span></span>
...
<li>
<p>将生成的hash id放到css元素上</p>
<ol>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">if</span> <span class="token punctuation">(</span>hasScoped<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<li>
<p>暴漏文件名利于devtools工具</p>
<ol>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// Expose filename. This is used by the devtools and Vue runtime warnings.</span>
...
<ol>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">//如果propsToAttach存在，添加else代码来Tree  shaking</span>
...
<ol>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">//添加热重载的代码</span>
...
<li>
<p>attrsToQuery这个函数主要用于获取用户传入的参数并忽略用户传入的内置使用的属性const ignoreList = ['id', 'index', 'src', 'type']</p>
</li>
</ol>
<p>总结：==vue-loader主要做的就是解析Vue文件，分别生成js css html，并分别进行相对应的处理。并做热更新和摇树的优化操作。==</p>
<h2 id="_2-vueloaderplugin" tabindex="-1"><a class="header-anchor" href="#_2-vueloaderplugin" aria-hidden="true">#</a> 2.VueLoaderPlugin</h2>
<p>在使用Vueloader的时候必须要在webpack.config.js的plugins中注册该插件。</p>
<p>这个插件的作用主要是将Vue分割出来的js html css都有对应的loader去处理。</p>
