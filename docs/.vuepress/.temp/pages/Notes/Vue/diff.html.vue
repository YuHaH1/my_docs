...
<p>diff算法本质上是为了提升效率，用js计算为代价来减少对DOM的操作。通过比较新旧VNode来尽可能最小化DOM操作来实现最小化性能开销。</p>
<p>key的作用，如果没有key我们就无法知道新子节点和旧子节点的映射关系，就无法知道如何移动节点，有key节点才能知道新旧节点的位置。</p>
<p>源码位置<code v-pre>src/core/vdom/patch.ts</code>最重要的key和tag是否相同</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="vue2diff" tabindex="-1"><a class="header-anchor" href="#vue2diff" aria-hidden="true">#</a> vue2diff</h2>
<h3 id="核心思路" tabindex="-1"><a class="header-anchor" href="#核心思路" aria-hidden="true">#</a> 核心思路</h3>
<p>DOM的核心就是patch，DOM操作执行速度远不如js的运算速度快。</p>
<ol>
<li>同级比较</li>
<li>首次渲染不存在oldVnode直接使用vNode创建元素并渲染视图</li>
<li>看两节点是否为同一节点，如果不是直接创建新节点替换旧节点</li>
</ol>
<ul>
<li>只有元素、注释和文本节点会被创建并插入到DOM中（如果有tag属性就认为是元素节点，如果没有tag属性就看有没有isComment属性看是否是注释节点如果是注释节点则调用createComment，如果也没有就是文本节点直接通过createTextNode创建文本节点）</li>
</ul>
<ol start="4">
<li>
<p>如果是同一个节点则判断节点tag和key，比较两个节点属性是否相同（复用老的节点将有差异的属性进行更新 ）</p>
</li>
<li>
<p>比较属性时例如比较style，如果新旧节点都存在的属性直接复用，新结点上不存在的就删掉</p>
</li>
<li>
<p>然后比较子节点，看是否都存在子节点，</p>
</li>
</ol>
<ul>
<li>
<p>如果老节点没子，新节点有直接插入</p>
</li>
<li>
<p>如果老节点有子，新节点没子直接删除</p>
</li>
<li>
<h2 id="如果都存在子节点继续diff算法" tabindex="-1"><a class="header-anchor" href="#如果都存在子节点继续diff算法" aria-hidden="true">#</a> <strong>如果都存在子节点继续DIFF算法</strong></h2>
<ul>
<li>
<p>为了提高性能，采用双指针方法，四种对比方式（1.新前与旧前 2.新后与旧后 3.新后与旧前 4.新前与旧后）首先准备四个变量oldStartIndex、oldEndIndex、newStartIndex和newEndIndex。在循环体内每处理一个节点就将下标朝一个方向移动一次，开始位置索引向后，结尾位置索引向前，<strong>当开始位置元素索引大于等于结束位置的时候，说明所有节点都遍历过了，则结束循环，这样保证了循环体内的元素都是未处理元素</strong>。</p>
<ul>
<li>
<p>循环条件</p>
</li>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">while</span><span class="token punctuation">(</span>oldStartIndex<span class="token operator">&lt;=</span>oldEndIndex <span class="token operator">&amp;&amp;</span> newStartIndex<span class="token operator">&lt;=</span>newEndIndex<span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul>
<li>当newChildren或oldChildren有一个循环完毕就结束循环，目的是为了提高性能找出差异即可，如果oldChildren先循环完，那么newChildren还有节点未处理，说明需要新增newChildren中剩余未处理节点</li>
<li>如果newChildren先循环完，oldChildren还剩余未处理节点说明oldChildren中的节点是废弃节点需要删除，这样做的好处是避免了多次循环</li>
<li>如何判断oldChildren先循环完还是newChildren先循环结束？ <strong>当oldChildren的oldStartIndex&lt;oldEndIndex说明新节点还有未处理的需要新增。当newStartIndex&lt;newEndIndex说明oldChildren剩余未处理节点要删除</strong></li>
</ul>
</li>
</ul>
</li>
...
<p>先新前与旧前指针进行比较</p>
<ul>
<li><img src="/Vue/diff4.png" alt=""></li>
<li>如果同一个节点则直接复用也不需要移动位置，然后两个指针都向后移动，</li>
<li>如果不是则新后与旧后比较</li>
</ul>
</li>
<li>
<p>新后与旧后</p>
<ul>
<li><img src="/Vue/diff3.png" alt=""></li>
<li>如果是同一节点直接复用，执行更新操作不需要移动位置，然后指针向前移。</li>
<li>如果不是则新后与旧前</li>
</ul>
</li>
<li>
<p>新后与旧前</p>
<ul>
<li><img src="/Vue/diff2.png" alt=""></li>
<li>如果是同一节点则需要移动节点，将旧前位置的节点移动到所有<strong>未处理节点之前</strong>（不能移动到已处理节点之后，因为顺序会错乱下一个如果也是新后旧前同一节点，插入到已处理的后面就会导致上一个节点成了最后面的了，<strong>因此应该插入到未处理节点之后</strong>）</li>
<li>如果不是同一节点则新前与旧后</li>
</ul>
</li>
<li>
<p>新前与旧后</p>
<ul>
<li><img src="/Vue/diff1.png" alt=""></li>
<li>如果是同一节点，则需要将旧后移动到所有未处理节点之前。</li>
</ul>
</li>
<li>
<p>大部分通过前四部就可以找到相同节点去复用，因此可以节省很多次循环，如果一个节点经过前四部依然没找到，则再循环旧节点看有无能复用的。</p>
</li>
</ul>
</li>
</ul>
...
<span class="token comment">//比较是否是同一个节点</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="vue3diff" tabindex="-1"><a class="header-anchor" href="#vue3diff" aria-hidden="true">#</a> Vue3diff</h2>
<p>Vue3快速diff借鉴了纯文本diff的思路，也就是前后缀比较</p>
<ul>
<li>
<p>**首尾比较：**先新前和旧前相比较，如果节点相同就patch，然后继续比较直到遇到不同的结束循环,然后开始新后和旧后相比，遇到不同的循环终止</p>
<ul>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 头比：新前和旧前</span>
...
<li>
<p>对于乱序情况，先初始化一个值全部为-1的数组，长度为新的子节点中的未处理节点数量,这个数组是为了后续求最长递增子序列。</p>
<ul>
<li>
...
<li>
<p>然后初始化后将<u>该数组存储新节点在结点中的索引位置，如果旧节点中不存在该节点意味着是新增节点值仍为-1</u>。<strong>为了避免双重循环采用一个对象作为索引表</strong></p>
<ul>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> oldStart <span class="token operator">=</span> j
...
<li>
<p><img src="/Vue/diff.jpg" alt=""></p>
</li>
</ul>
</li>
<li>
<p>新增<code v-pre>move</code> 和 <code v-pre>pos</code> 变量。前者初始化为<code v-pre>fasle</code>，后者代表需要移动节点的位置初始化为<code v-pre>0</code>。以及<code v-pre>patched</code>表示已经更新过节点的数量。（已经更新过的节点数量应该小于新的子节点中需要更新的数量，否则说明存在多余节点要卸载）</p>
<ul>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> oldStart <span class="token operator">=</span> j
<span class="token keyword">const</span> newStart <span class="token operator">=</span> j
...
<li>
<p>source中的-1 变成了 i也就是老节点中的位置</p>
</li>
</ul>
</li>
<li>
<p>当move为true就意味要移动元素开始最长递增子序列。这个子序列的元素意味着不需要移动位置。（该子序列存放的是source递增子序列元素的下标）</p>
<ul>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">if</span><span class="token punctuation">(</span>move<span class="token punctuation">)</span><span class="token punctuation">{</span>
...
<li>
<p><img src="/Vue/diff2.jpg" alt=""></p>
</li>
</ul>
</li>
<li>
<p>当需要移动时，查看节点在source中的值是否为-1，-1就说明是新增节点</p>
<ul>
<li>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">if</span><span class="token punctuation">(</span>source<span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token operator">===</span><span class="token operator">-</span><span class="token number">1</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
      <span class="token comment">// 对于p7节点，i = 3 + 1 = 4</span>
      <span class="token keyword">const</span> pos <span class="token operator">=</span> i <span class="token operator">+</span> newStart
...
<li>
<p>如果source[i]!==-1，-1就说明是可复用节点需要移动</p>
<ul>
<li>
...
<li>
<p>其他情况就是i ===  seq[j]也就是该节点在子序列里不需要移动只需要s-=1即可</p>
<ul>
<li>
